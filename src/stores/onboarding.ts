...
export const useOnboardingStore = defineStore('onboarding', () => {
  ...
  const formattedPromptBlock = computed(() => {
    ...
    sections.push('\n## STAR 表达规则\n- 项目经历请尽量按 STAR 逻辑组织：Situation/Task 说明真实背景或目标，Action 说明用户采取的具体行动，Result 说明真实结果或影响。\n- 最终简历不要机械显示 “S/T/A/R” 字母标题，应用自然的中文模块名，例如“项目简介”“职责与落地”“关键成果”。\n- 如果用户没有填写某个 STAR 环节，不要编造；可以省略该环节或用已提供信息弱化表达。\n- 结果部分只能使用用户提供的真实结果和指标；没有量化数据时，使用保守的非量化影响表达。')
